#!/usr/bin/env python3
"""
太阳能电站功率预测 - RoPE ED Transformer 训练脚本 v3

基于 solar_train_rope_v2.py, 改动:
- 评估部分: 显示 16 个预测点的逐点准确度 (替代原来的 4h 时间段汇总)

使用方式:
  python solar_train_rope_v3.py                          # 完整流程
  python solar_train_rope_v3.py --mode train             # 仅训练
  python solar_train_rope_v3.py --mode evaluate          # 仅评估
  python solar_train_rope_v3.py --noise-std 0.02         # 调整噪声强度
"""
...
def train(args):
    ...
    # 模型 (缩小版: 默认 d_model=128, 3+2 layers, 4 heads)
    ...
    # TensorBoard
    log_dir = os.path.join("runs", f"rope_v3_{time.strftime('%Y%m%d_%H%M%S')}")
    ...
    for epoch in epoch_bar:
        ...
        if acc2 > best_acc2:
            ...
            torch.save({
                ...
                "args": vars(args),
            }, "solar_checkpoints/best_model_rope_v3.pth")
...
def evaluate(args):
    ...
    checkpoint = torch.load("solar_checkpoints/best_model_rope_v3.pth",
...
def main():
    parser = argparse.ArgumentParser(description="太阳能电站功率预测 (RoPE v3 逐点评估)")
    ...
    parser.add_argument("--lr", type=float, default=1e-4,
                        help="学习率 (降低至 1e-4, 减少 val loss 尖峰)")
    parser.add_argument("--weight-decay", type=float, default=0.05,
                        help="L2正则化系数 (加大至 0.05)")
