#!/usr/bin/env python3
"""
太阳能电站功率预测 - GRU Encoder-Decoder 训练脚本 (混合 Loss 版)

混合 Loss = λ_mse * MSE + λ_acc2 * ACC2_Loss
  - MSE:       关注绝对误差, 大功率时段梯度大
  - ACC2 Loss: 关注相对误差, 与国标 ACC2 评估指标直接对齐

准确度指标 (与 solar_train_ed.py 保持一致):
  - ACC1: MAE-based, 1 - MAE / mean(y_true)
  - ACC2: 国标, 1 - sqrt((1/N) * sum(((P_M - P_P) / max(P_M, 0.2*Cap))^2))
  - RMSE: 均方根误差 (MW)
  - MAE:  平均绝对误差 (MW)

注意: 本脚本不屏蔽夜晚功率等于0的时刻 (所有样本均参与训练和评估)

使用方式:
  python gru_train.py                           # 完整流程: 预处理 + 训练 + 评估
  python gru_train.py --mode train              # 仅训练
  python gru_train.py --mode evaluate           # 仅评估
  python gru_train.py --hidden-size 128 --num-layers 3  # 调整模型结构
"""
...
from gru_model import SolarGRU
...
class MixedLoss(nn.Module):
    ...
    def __init__(self, lambda_mse: float = 1.0, lambda_acc2: float = 0.5,
                 cap_norm: float = 1.0):
        super().__init__()
...
def train(args):
    ...
    print(f"标称容量: {cap} MW")
    print("注意: 不屏蔽夜晚功率等于0的时刻, 所有样本均参与训练")
    ...
    # TensorBoard
    log_dir = os.path.join("runs", f"gru_{time.strftime('%Y%m%d_%H%M%S')}")
    ...
    print(f"模型: hidden_size={args.hidden_size}, num_layers={args.num_layers}, "
    ...
    for epoch in epoch_bar:
        ...
        if val_loss < best_val_loss:
            ...
            torch.save({
                ...
                "args": vars(args),
            }, "solar_checkpoints/best_model_gru.pth")
...
def evaluate(args):
    ...
    checkpoint = torch.load("solar_checkpoints/best_model_gru.pth",
...
def main():
    parser = argparse.ArgumentParser(
        description="太阳能电站功率预测 (GRU Encoder-Decoder + 混合 Loss)"
    )
