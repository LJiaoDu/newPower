#!/usr/bin/env python3
"""
太阳能电站功率预测 - 训练与评估脚本 (改进版)

改进点:
1. 适配 Encoder-Decoder 双输入 (X_enc + X_dec)
2. 使用 Huber Loss 替代 MSE, 对异常点更鲁棒
3. 支持 ACC1 / ACC2 / RMSE / MAE 全指标评估
4. TensorBoard 日志
...
def train(args):
    ...
    print(f"开始训练 | Epochs: {args.epochs} | Batch: {args.batch_size} | "
          f"LR: {args.lr} | Loss: HuberLoss(delta={args.huber_delta})")
...
def evaluate(args):
    ...
    checkpoint = torch.load("solar_checkpoints/best_model.pth",
...
def main():
    parser = argparse.ArgumentParser(description="太阳能电站功率预测 (Encoder-Decoder Transformer)")
