#!/usr/bin/env python3
"""
太阳能电站功率预测 - Encoder-Only Transformer 训练脚本

基于 solar_train_rope_v2.py 的训练策略:
- 缩小模型: d_model=128, nhead=4, num_layers=4
- 加大正则: dropout=0.2, weight_decay=0.05
- 降低学习率: lr=1e-4
- 保存标准: ACC2 最高
- 输入噪声增强
- 混合 Loss: MSE + ACC2
...
改动:
- 模型替换为 SolarTransformerEncOnly (Encoder-Only + RoPE)
- 评估显示 16 个预测点的逐点准确度

使用方式:
  python solar_train_enc_only.py                          # 完整流程
  python solar_train_enc_only.py --mode train             # 仅训练
  python solar_train_enc_only.py --mode evaluate          # 仅评估
"""
...
def train(args):
    ...
    # TensorBoard
    log_dir = os.path.join("runs", f"enc_only_{time.strftime('%Y%m%d_%H%M%S')}")
    ...
    for epoch in epoch_bar:
        ...
        if acc2 > best_acc2:
            ...
            torch.save({
                ...
                "args": vars(args),
            }, "solar_checkpoints/best_model_enc_only.pth")
...
def evaluate(args):
    ...
    checkpoint = torch.load("solar_checkpoints/best_model_enc_only.pth",
...
def main():
    parser = argparse.ArgumentParser(description="太阳能电站功率预测 (Encoder-Only + RoPE)")
