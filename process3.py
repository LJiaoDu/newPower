...
def load_site_df(path: str):
    ...
    if ext in (".xlsx", ".xls"):
        ...
        for col in weather_cols:
            ...
            if n_bad > 0:
                df_raw[col] = df_raw[col].replace(-99, float("nan"))
                print(f"  {col}: 替换 {n_bad} 个 -99 值")
        # rh 必须在转为数值后再比较，避免 object 列 TypeError
        rh_numeric = pd.to_numeric(df_raw["rh"], errors="coerce")
        n_rh_bad = int((rh_numeric > 100).sum())
        if n_rh_bad > 0:
            df_raw["rh"] = rh_numeric.where(rh_numeric <= 100, other=float("nan"))
