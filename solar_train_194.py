...
def train(args):
    ...
    train_loader = DataLoader(
        TensorDataset(torch.FloatTensor(X_enc_train),
                      torch.FloatTensor(X_dec_train),
                      torch.FloatTensor(y_train)),
        batch_size=args.batch_size, shuffle=True, num_workers=2, pin_memory=True,
    )
    val_loader = DataLoader(
        TensorDataset(torch.FloatTensor(X_enc_val),
    ...
    criterion = MixedLoss(lambda_mse=args.lambda_mse,
    ...
    print(f"Loss: λ_mse={args.lambda_mse}*MSE + λ_acc2={args.lambda_acc2}*ACC2")
    print(f"特征: Encoder={enc_feat_size}维, Decoder={dec_feat_size}维")
    print(f"{'='*70}\n")
...
def evaluate(args):
    ...
    ckpt = torch.load("solar_checkpoints/best_model_194.pth",
                      map_location=device, weights_only=False)
    model.load_state_dict(ckpt["model_state_dict"])
    model.eval()
    print(f"加载模型: epoch={ckpt['epoch']+1}, ACC2={ckpt['acc2']:.4f}")

    test_loader = DataLoader(
        TensorDataset(torch.FloatTensor(X_enc_test),
    ...
    print(f"  {'步':>3}  {'时刻':>7}  {'ACC1':>7}  {'ACC2':>7}  {'RMSE(MW)':>9}  {'MAE(MW)':>8}")
    print(f"  {'-'*3}  {'-'*7}  {'-'*7}  {'-'*7}  {'-'*9}  {'-'*8}")
    for step in range(num_steps):
        ...
        print(f"  {step+1:3d}  {'+%dmin' % ((step+1)*15):>7}  "
