#!/usr/bin/env python3
"""
太阳能电站功率预测 - Encoder-Decoder Transformer + RoPE (training_data.csv 版本)

与 rope_model_1.py 的主要区别:
1. 数据源: training_data.csv (无气象特征)
2. Encoder 输入: [B, 288, 7]  历史24h (功率+时间) @ 5分钟粒度
3. Decoder 输入: [B, 48,  6]  未来4h  (时间)       @ 5分钟粒度
4. 输出:         [B, 48]      未来功率预测

架构 (与 rope_model_1.py 完全相同, 仅参数不同):
  Encoder: 历史24h (功率+时间) [B, 288, 7] -> RoPE Self-Attention -> memory
  Decoder: 未来4h  (时间)      [B, 48,  6] -> RoPE Self-Attn + Cross-Attn -> 功率预测
"""
...
class RotaryPositionEncoding(nn.Module):
    ...
    def __init__(self, head_dim: int, max_len: int = 400, base: float = 10000.0):
        super().__init__()
...
class SolarTransformerRoPE(nn.Module):
    ...
    Encoder 输入: [B, 288, 7]  历史 (时间6 + 功率1)  @ 5分钟粒度
    Decoder 输入: [B, 48,  6]  未来 (时间6, 无功率)  @ 5分钟粒度
    输出:         [B, 48]      未来功率预测
    """
    ...
    def forward(self, x_enc: torch.Tensor, x_dec: torch.Tensor) -> torch.Tensor:
        ...
        # 1. 预计算 RoPE cos/sin
        enc_cos, enc_sin = self.rope(enc_len, offset=0)           # [0..287]
        dec_self_cos, dec_self_sin = self.rope(dec_len, offset=0)  # [0..47] 局部位置
        dec_cross_q_cos, dec_cross_q_sin = self.rope(dec_len, offset=enc_len)  # [288..335]
        dec_cross_k_cos, dec_cross_k_sin = enc_cos, enc_sin       # [0..287]

        # 2. Encoder
        enc = self.enc_projection(x_enc)     # [B, 288, d_model]
        for layer in self.encoder_layers:
            enc = layer(enc, enc_cos, enc_sin)
        memory = self.encoder_norm(enc)      # [B, 288, d_model]

        # 3. Decoder
        dec = self.dec_projection(x_dec)     # [B, 48, d_model]
        for layer in self.decoder_layers:
            dec = layer(dec, memory,
                        dec_self_cos, dec_self_sin,
                        dec_cross_q_cos, dec_cross_q_sin,
                        dec_cross_k_cos, dec_cross_k_sin)
        out = self.decoder_norm(dec)         # [B, 48, d_model]

        # 4. 输出
        pred = self.output_projection(out)   # [B, 48, 1]
        return pred.squeeze(-1)              # [B, 48]
...
if __name__ == "__main__":
    ...
    x_enc = torch.randn(4, 288, 7).to(device)
    x_dec = torch.randn(4, 48, 6).to(device)
    with torch.no_grad():
