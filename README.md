...
- **数据**: 13个月历史数据（393个文件），5分钟间隔，114,215个数据点
...
**Q: 为什么数据从114,215行变成64,664行？**
A: 因为创建滞后特征(lag_288)和滚动窗口特征需要足够的历史数据点，所以前面的数据被删除了。这是正常的，确保每个训练样本都有完整的特征。

**Q: 如何获得最佳效果？**
A: 直接运行 `python improved_train.py`，它已经包含了所有优化。

**Q: 需要多少数据？**
A: 当前使用13个月数据(393个文件)效果非常好(98.91%准确率)。数据越多，模型越稳定准确。
