...
架构对比:
  ...
  LSTM Baseline (solar_lstm_baseline.py):
  ...
  GRU Baseline (本文件):
    GRU(96步,13feat) -> 最后隐状态[B,H]   # GRU 无 cell state, 参数更少
                              +
    未来天气展平[B,16*12=192]
                              ↓
    Concat -> FC -> [B,16]
...
class SimpleGRU(nn.Module):
    """
    步骤:
      1. GRU 处理历史序列 X_enc [B, 96, 13] -> 最后隐状态 [B, hidden]
         (GRU 只有 h, 无 cell state c, 参数量比 LSTM 少约 25%)
...
def train(args):
    ...
    print(f"\n模型参数量: {total_params:,}")
    print(f"结构: GRU(enc={enc_input}, h={args.hidden_size}, L={args.num_layers})"
...
def evaluate(args):
    ...
    ckpt = torch.load("gru_baseline_ckpt/best_model.pth",
    ...
    print_metrics(all_trues, all_preds, cap,
                  label="测试集评估结果 (SimpleGRU Baseline)")
