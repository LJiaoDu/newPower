#!/usr/bin/env python3
"""
太阳能电站功率预测 - 简单 GRU Baseline (solar_station_1.csv)

GRU Baseline:
  GRU(96步,13feat) -> 最后隐状态[B,H]
                          +
  未来天气展平[B,16*12=192]
                          ↓
  Concat -> FC -> [B,16]
...
def save_log_to_file():
    ...
    try:
        ...
        filename = f"solar_logs/final_log_gru_baseline_{time_str}.txt"
...
# =====================================================================
# 数据预处理 (与 solar_lstm_baseline.py 相同逻辑, 共用 norm_params.pkl)
# =====================================================================
...
def print_metrics(y_true, y_pred, cap, label="评估结果"):
    ...
    for i in range(16):
        ...
        log_print(f"    点{i+1:2d} ({tlabel:>7s}): "
                  f"ACC1={s_acc1:.4f}, "
                  f"ACC2={s_acc2:.4f}, "
...
class SimpleGRU(nn.Module):
    ...
    def forward(self, x_enc, x_dec):
        out, _ = self.gru(x_enc)                     # [B, 96, hidden]
        last = out[:, -1, :]                         # [B, hidden]
...
def train(args):
    ...
    enc_input = X_enc_train.shape[2]
    dec_flat  = X_dec_train.shape[1] * X_dec_train.shape[2]
    ...
    log_print(f"\n模型参数量: {total_params:,}")
    log_print(f"结构: GRU(enc={enc_input}, h={args.hidden_size}, L={args.num_layers}) + concat(dec_flat={dec_flat}) + FC -> {out_steps}")
    ...
    for epoch in epoch_bar:
        ...
        if val_loss < best_val_loss:
            ...
            torch.save({
                ...
                "args": vars(args),
            }, "gru_baseline_ckpt/best_model.pth")
...
def evaluate(args):
    ...
    ckpt = torch.load("gru_baseline_ckpt/best_model.pth", map_location=device, weights_only=False)
    saved = ckpt.get("args", {})
    ...
    print_metrics(all_trues, all_preds, cap, label="测试集评估结果 (SimpleGRU Baseline)")
...
def main():
    parser = argparse.ArgumentParser(description="简单 GRU Baseline - solar_station_1.csv")
