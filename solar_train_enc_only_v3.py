#!/usr/bin/env python3
"""
太阳能电站功率预测 - Encoder-Only Transformer 训练脚本 (v3 - 含 TSI 衍生特征)

在 solar_train_enc_only.py 基础上, 使用 solar_preprocess_v3 进行数据预处理:
  - Encoder 输入: 16维 (时间6 + 气象6 + tsi_diff1 + tsi_diff2 + tsi_std_4 + 功率1)
  - Decoder 输入: 15维 (时间6 + 气象6 + tsi_diff1 + tsi_diff2 + tsi_std_4)

新增 TSI 衍生特征说明:
  - tsi_diff1  : TSI 一阶差分 (辐照变化速率, MaxAbs 归一化到 [-1,1])
  - tsi_diff2  : TSI 二阶差分 (辐照变化加速度, MaxAbs 归一化到 [-1,1])
  - tsi_std_4  : TSI 滚动标准差 (窗口=4步=1小时, 辐照波动程度, Min-Max 归一化)

使用方式:
  python solar_train_enc_only_v3.py                          # 完整流程
  python solar_train_enc_only_v3.py --mode train             # 仅训练
  python solar_train_enc_only_v3.py --mode evaluate          # 仅评估
"""
...
from solar_model_enc_only import SolarTransformerEncOnly
from solar_preprocess_v3 import main as preprocess_main
...
def train(args):
    ...
    print(f"特征: Encoder {enc_feat_size}维 (含tsi_diff1/diff2/std4), "
          f"Decoder {dec_feat_size}维 (含tsi_diff1/diff2/std4)")
    ...
    # TensorBoard
    log_dir = os.path.join("runs", f"enc_only_v3_{time.strftime('%Y%m%d_%H%M%S')}")
    ...
    print(f"模型: SolarTransformerEncOnly | d_model={args.d_model}, heads={args.nhead}, "
          f"layers={args.num_layers}")
    print(f"序列: 历史{enc_seq_len}步 + 未来{dec_seq_len}步 = {enc_seq_len+dec_seq_len}步 全局 Self-Attention")
    print(f"正则: dropout={args.dropout}, weight_decay={args.weight_decay}")
    print(f"输入噪声: std={args.noise_std}")
    print(f"保存标准: ACC2 最高")
    print(f"{'='*70}\n")
    ...
    for epoch in epoch_bar:
        ...
        if acc2 > best_acc2:
            ...
            torch.save({
                ...
                "args": vars(args),
            }, "solar_checkpoints/best_model_enc_only_v3.pth")
...
def evaluate(args):
    ...
    checkpoint = torch.load("solar_checkpoints/best_model_enc_only_v3.pth",
    ...
    print(f"\n{'='*60}")
    print(f"测试集评估结果 (Encoder-Only v3, 含TSI衍生特征)")
    print(f"{'='*60}")
...
def main():
    parser = argparse.ArgumentParser(
        description="太阳能电站功率预测 (Encoder-Only + RoPE + TSI衍生特征)"
    )
