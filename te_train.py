#!/usr/bin/env python3
"""
新数据格式训练脚本 - 配合te_data_process.py使用

与原train.py的主要区别：
1. 默认数据路径改为 te_training_data.csv
2. 检查点和日志目录使用 'te_' 前缀
3. 其他训练逻辑保持不变

使用方法：
1. 先运行 te_data_process.py 生成 te_training_data.csv
2. 再运行此脚本进行训练
"""
...
def train_val(cfg):
    ...
    writer = SummaryWriter(log_dir=f'runs/te_training_{timestamp}')
    ...
    # 创建检查点目录
    os.makedirs('./te_checkpoint', exist_ok=True)
    ...
    for epoch_i in range(start_epoch, cfg.epochs):
        ...
        writer.add_text(
            "Validation Summary",
            (
                f"**Epoch {epoch_i} Summary (New Format)**\n\n"
                f"- Model: ImprovedTFMModel (non-autoregressive)\n"
                f"- Data: te_training_data.csv (new format)\n"
                f"- LR: {lr:.8f}\n"
        ...
        torch.save(checkpoint, f"./te_checkpoint/checkpoint_epoch{epoch_i}.pth")
        ...
        if val_loss < best_val_loss:
            ...
            patience_counter = 0
            torch.save(checkpoint, f"./te_checkpoint/best_epoch{epoch_i}.pth")
    ...
    print("="*80)
    print(f"最佳验证损失: {best_val_loss:.6f}")
    print(f"检查点目录: ./te_checkpoint/")
    print("="*80)


def get_args():
    parser = argparse.ArgumentParser(
        description='新格式数据训练脚本',
    ...
    parser.add_argument('--data-path', type=str, default='./te_training_data.csv',
                        help='训练数据路径 (新格式)')

    # 学习率参数
    parser.add_argument('--lr-init', type=float, default=0.0004, help='初始学习率')
