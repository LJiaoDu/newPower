#!/usr/bin/env python3
"""
太阳能电站功率预测 - RoPE Encoder-Decoder 训练脚本

模型: SolarTransformerRoPE (RoPE 应用在 Q/K 上, 非 additive PE)
Loss: 混合 Loss = λ_mse * MSE + λ_acc2 * ACC2_Loss
数据: 不过滤夜间样本, 使用全量数据训练

使用方式:
  python solar_train_rope.py                          # 完整流程: 预处理 + 训练 + 评估
  python solar_train_rope.py --mode train             # 仅训练
  python solar_train_rope.py --mode evaluate          # 仅评估
  python solar_train_rope.py --lambda-mse 1.0 --lambda-acc2 0.5  # 调整 loss 权重
"""
...
def train(args):
    ...
    # TensorBoard
    log_dir = os.path.join("runs", f"rope_{time.strftime('%Y%m%d_%H%M%S')}")
    ...
    for epoch in epoch_bar:
        ...
        if val_loss < best_val_loss:
            ...
            torch.save({
                ...
                "args": vars(args),
            }, "solar_checkpoints/best_model_rope.pth")
...
def evaluate(args):
    ...
    checkpoint = torch.load("solar_checkpoints/best_model_rope.pth",
...
def main():
    parser = argparse.ArgumentParser(description="太阳能电站功率预测 (RoPE ED Transformer + 混合 Loss)")
